...
function createTileTexture(tile, isCorner) {
  ...
  if (tile.type === "property" && groupInfo) {
    // Saturated Property Color Bar at top (inner edge towards center felt)
    ...
    // Price tag at bottom (outer edge near player)
...
export default function Board3D({
  ...
  useEffect(() => {
    ...
    // 4. Build 40 Tile Meshes: 100% Upright Authentic Rotations
    ...
    BOARD_TILES.forEach((tile) => {
      ...
      // PRECISE CARD ORIENTATIONS:
      if (tile.id === 0) {
        mesh.rotation.y = 0; // GO upright
      } else if (tile.id >= 1 && tile.id <= 9) {
        mesh.rotation.y = 0; // Bottom row: Text upright, Color bar at inner felt
      } else if (tile.id === 10) {
        mesh.rotation.y = 0; // JAIL upright
      } else if (tile.id >= 11 && tile.id <= 19) {
        mesh.rotation.y = -Math.PI / 2; // Left row: Text upright, Color bar at inner felt
      } else if (tile.id === 20) {
        mesh.rotation.y = Math.PI; // FREE PARKING upright
      } else if (tile.id >= 21 && tile.id <= 29) {
        mesh.rotation.y = Math.PI; // Top row: Text upright, Color bar at inner felt
      } else if (tile.id === 30) {
        mesh.rotation.y = Math.PI; // GO TO JAIL upright
      } else if (tile.id >= 31 && tile.id <= 39) {
        mesh.rotation.y = Math.PI / 2; // Right row: Text upright, Color bar at inner felt
      }
  ...
  useEffect(() => {
    ...
    Object.keys(properties).forEach((tileIdStr) => {
      ...
      // 3D Ownership Ribbon placed cleanly at the OUTER PRICE EDGE (away from color bar)
      if (prop.owner) {
        ...
        // Position on the OUTER RIM edge
      ...
      // 3D Houses / Hotels placed squarely on the INNER COLOR BAR (bordering center green felt)
      if (prop.houses > 0) {
