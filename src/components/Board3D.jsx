...
function createTileTexture(tile, isCorner) {
  ...
  if (tile.type === "property" && groupInfo) {
    // Saturated Property Color Bar at top (outer perimeter)
    ...
    // Price tag at bottom (inner center felt)
...
export default function Board3D({
  ...
  useEffect(() => {
    ...
    // 4. Build 40 Tile Meshes: Color Bar is ALWAYS on OUTER RIM, Text faces INWARD towards center
    ...
    BOARD_TILES.forEach((tile) => {
      ...
      // ALL 4 SIDES HAVE COLOR BARS ON OUTER RIM, TEXT FACING INWARD:
  ...
  useEffect(() => {
    ...
    Object.keys(properties).forEach((tileIdStr) => {
      ...
      // 3D Ownership Ribbon placed cleanly on the INNER PRICE EDGE (near center green felt)
      if (prop.owner) {
